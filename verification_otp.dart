...
class OTPController extends ChangeNotifier {
  ...
  Future<void> verifyOTP(BuildContext context) async {
    ...
    try {
      ...
      String otp = otpCode;
      // for demon purpose will chnage based on the code of the real otp      
      if (otp == '1234') {
        if (context.mounted) {
      ...
      if (!_disposed) setErrorMessage('An error occurred during verification');
    } finally {
  ...
  Future<void> resendOTP() async {
    ...
    try {
      ...
      if (!_disposed) setErrorMessage('Failed to resend OTP. Please try again.');
    } finally {
      if (!_disposed) setResending(false);
    }
  }
